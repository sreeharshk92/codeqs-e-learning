...
export default function WorkshopVideoList() {
  ...
  const VideoPlayer = ({ videoUrl }) => (
    <video controls width="250">
      <source src={`http://127.0.0.1:8000/storage/${videoUrl}`} type="video/mp4" />
          ...
          {categorizedProducts.map((category) =>
            category.products.map((product) => (
              <tr key={`${category.id}-${product.video_id}`}>
                ...
                <td>
                  {product.google_meet_link ? (
                    <a
                      href={product.google_meet_link}
                      target="_blank"
                ...
                <td>{product.google_meet_topic || "N/A"}</td>
                <td>{formatDate(product.google_meet_scheduled_at)}</td>
